...
bool WwiseConsoleAkSoundDataBuilder::readBankData(FByteBulkData& bulkBankData, const FString& bankFile, IPlatformFile& fileManager)
{
			...
			auto readBankDataTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, FullPath, platform, requiredData]
				{
					...
					{
						if (sharedThis->readBankData(requiredData->eventPlatformData->Data, FullPath, *sharedThis->platformFile))
						{
			...
			if (auxBusPlatformData)
			{
				if (isBank)
				{
					auto readBankTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, akAuxBus, auxBusPlatformData, FullPath] {
						if (sharedThis->readBankData(auxBusPlatformData->Data, FullPath, *sharedThis->platformFile))
						{
			...
			auto readBankDataTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, FullPath, platform, requiredData]
				{
					...
					{
						if (sharedThis->readBankData(requiredData->audioBankPlatformData->Data, FullPath, *sharedThis->platformFile))
						{
	...
	else if (fileName == AkInitBankName || fileName == AkPluginInfoFileName)
	{
		...
		if (isBank)
		{
			auto readBankTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, initBank, initBankPlatformData, FullPath] {
				if (sharedThis->readBankData(initBankPlatformData->Data, FullPath, *sharedThis->platformFile))
				{
