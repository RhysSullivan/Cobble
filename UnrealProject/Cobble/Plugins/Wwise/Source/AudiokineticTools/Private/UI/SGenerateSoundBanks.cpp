...
void SGenerateSoundBanks::Construct(const FArguments& InArgs)
{
	...
	{
		OpenMsgDlgInt(EAppMsgType::Ok, NSLOCTEXT("AkAudio", "Warning_Ak_PlatformSupported", "Unable to generate Sound Data. Please select a valid Wwise supported platform in the 'Project Settings > Project > Supported Platforms' dialog."), LOCTEXT("AkAudio_Error", "Error"));
		return;
	}
...
void SGenerateSoundBanks::PopulateList(void)
{
	...
	for (const auto& Platform : AvailablePlatformNames)
	{
		...
		if (wwiseProjectInfo.SupportedPlatforms().Contains(PlatformInfo->WwisePlatform))
		{
